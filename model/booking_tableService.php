...
{
   return booking_table_Get("SELECT booking_table.id, booking_table.id_chung, booking_table.status, booking_table.name, booking_table.email, booking_table.phone, booking_table.address, booking_table.member, booking_table.booking_date, booking_table.booking_time, booking_table.content, booking_table.created FROM  booking_table ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function booking_table_insert($obj)
{
    return exe_query("insert into booking_table (id_chung,status,name,email,phone,address,member,booking_date,booking_time,content,created) values ('$obj->id_chung','$obj->status','$obj->name','$obj->email','$obj->phone','$obj->address','$obj->member','$obj->booking_date','$obj->booking_time','$obj->content','$obj->created')",'booking_table');
}
//
function booking_table_update($obj)
{
    return exe_query("update booking_table set id_chung='$obj->id_chung',status='$obj->status',name='$obj->name',email='$obj->email',phone='$obj->phone',address='$obj->address',member='$obj->member',booking_date='$obj->booking_date',booking_time='$obj->booking_time',content='$obj->content',created='$obj->created' where id=$obj->id",'booking_table');
}
//
