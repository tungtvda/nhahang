...
function view_danhgia($data)
{
    ...
    $ft->assign('TABLE-NAME','danhgia');
    $ft->assign('CONTENT-BOX-LEFT',isset($data['content_box_left'])?$data['content_box_left']:'');
    $ft->assign('CONTENT-BOX-RIGHT',isset($data['content_box_right'])?$data['content_box_right']:' ');
    $ft->assign('NOTIFICATION',isset($data['notification'])?$data['notification']:' ');
    $ft->assign('SITE-NAME',isset($data['sitename'])?$data['sitename']:SITE_NAME);
    $ft->assign('kichhoat_danhgia', 'active');
...
{
    return '<th>id</th><th>sanpham_id</th><th>name</th><th>email</th><th>phone</th><th>status</th><th>start</th><th>created</th>';
}
//
function showTableBody($data)
{
    ...
    if(count($data)>0) foreach($data as $obj)
    {
        ...
        $TableBody.="<td>".$obj->sanpham_id."</td>";
...
function showFrom($form,$ListKey=array())
{
    ...
    $str_from.='<p><label>sanpham_id</label><input class="text-input small-input" type="text"  name="sanpham_id" value="'.(($form!=false)?$form->sanpham_id:'').'" /></p>';
